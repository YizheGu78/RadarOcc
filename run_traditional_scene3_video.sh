...
OUTPUT_DIR="${OUTPUT_DIR:-$REPO_ROOT/work_dirs/tradition_occupancy_first_test_official_scene3}"
...
PYTHONPATH="$REPO_ROOT" python - "$OBJECT_MODEL" <<'PY'
import sys
...
echo "  layout    : $VIDEO_LAYOUT"
if [[ "$VIDEO_LAYOUT" == "both" ]]; then
    echo "              1) persistent | motion | RGB"
    echo "              2) Free / background / foreground + GT | RGB"
fi
echo "  fps       : $FPS"
...
echo "  unwrap    : range std $UNWRAP_RANGE_STD_M m; cluster radius $UNWRAP_CLUSTER_RADIUS_M m"
