...
def plot_bev(
    ...
    # RGB-aligned BEV:
    #   vertical   = +x forward
    #   figure left  = vehicle left (+y)
    #   figure right = vehicle right (-y)
    # Therefore horizontal display coordinate is -y.
    lateral_display = -xy[:, 1]
    forward = xy[:, 0]
    ...
    # Horizontal range is [-25.6, 25.6] in display coordinates (-y).
    ...
    # Ego vehicle at bottom center; forward direction matches the RGB view.
...
def main() -> None:
    ...
    fig.suptitle(
        "Qualitative static Doppler residual threshold comparison — RGB-aligned BEV"
